...
export async function pathNikita(pathname: string, DB: D1Database, body: any): Promise<Response> {
	...
	if (restPathname === '/' || restPathname === '') {
		return new Response(message, HEADERS.TEXT);
	}

	// If the pathname is /nikita/routes, return the routes
	if (restPathname === '/routes') {
		return await showRoutes(DB);
	}
	...
	if (idNameLink) {
		return await pathApi<BodyIdName>(DB, idNameLink, restPathname, body);
	}

	// If the pathname starts with /games, use the game body to get the data
	const gameLink = restPathname.startsWith('/games');
	if (gameLink) {
		return await pathApi<BodyGame>(DB, DB_NIKITA_TABLES.games, restPathname, body);
	}
	...
	if (gameListLink) {
		return await pathApi<BodyGameList>(DB, DB_NIKITA_TABLES.listGames, restPathname, body);
	}
	...
	if (gameGenreLink) {
		return await pathApi<BodyGameGenre>(DB, DB_NIKITA_TABLES.genreGames, restPathname, body);
	}
	...
	if (gamePlatformLink) {
		return await pathApi<BodyGamePlatform>(DB, DB_NIKITA_TABLES.platformGames, restPathname, body);
	}
